...
(function() {
  var check, current, house, my, opposite, sum, turn, your, ysum;
  ...
  App.room = App.cable.subscriptions.create("RoomChannel", {
    ...
    received: function(data) {
      if (String(data['group'].replace(/\r?\n/g, "")) === String(house)) {
        ...
        if (check === 0 && opposite === 0) {
          opposite = 2;
          your = data['message'].slice(47).slice(0, -14);
        } else if (check === 1 && opposite === 2) {
          check = 2;
          my = data['message'].slice(47).slice(0, -14);
        } else if (check === 1 && opposite === 0) {
          opposite = 1;
          my = data['message'].slice(47).slice(0, -14);
        } else if (check === 1 && opposite === 1) {
          check = 2;
          your = data['message'].slice(47).slice(0, -14);
        }
        if (check === 2) {
          ...
          if (parseInt(my, 10) > parseInt(your, 10)) {
            ...
            $('#point').empty();
            $('#point').append(sum);
          } else if (parseInt(my, 10) === parseInt(your, 10)) {
          ...
          if (turn === 6) {
            ...
            if (sum > ysum) {
              return $('#messages').append("<br><strong>あなたの勝利</strong>");
            } else if (ysum > sum) {
              return $('#messages').append("<br><strong>あなたの負け</strong>");
            } else if (sum === ysum) {
              return $('#messages').append("<br><strong>引き分け</strong>");
            }
          }
        }
      }
    },
    speak: function(message, group, check, house) {
      return this.perform('speak', {
        message: message,
        group: group,
        check: check,
